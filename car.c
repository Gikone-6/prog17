...
*
* CHANGES:
* 1.0 [47MAR2017 (FECHA HUMANA), 843jhdf03nj///jkeKEK9 (Fecha intergalactica general numero 6 estandard 3)] - Initial release
*/
...
void altaNuevaFila(){
  ...
  char enter, c;
  do {
    printf("Introduzca el coste del vehiculo:");
    read = scanf("%"xstr(max1)"[^\r\n]%c", precio,&enter);
    if(read != 2 || enter != '\n'){
      printf("Incorrect value, please try again.\n");
      clean_stdin();
      precio[0]= '\0';
    }
    else if (sizeof precio > max1) {
      printf("Value out of range, please try again.\n");
      precio[0]= '\0';
    }
    else {
